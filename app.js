...
/* Everything rendered through innerHTML goes through this first.
   The sample data below is trusted, but groupFeed() is designed to
   replace it with third-party RSS, where titles and Duelist names
   are attacker-controlled. Escape at the boundary, always. */
const ESCAPES = {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'};
...
/* ============================================================
   2. READING THE REAL FEED
   ------------------------------------------------------------
   The source blog runs WordPress, so it exposes RSS at /feed/
   (?paged=2 for older items). The hard part is not fetching it,
   it is that one tournament arrives as ~47 separate posts with
   titles like "Pairings for Round 11". These functions turn that
   flat stream back into events containing rounds.

   Browsers cannot fetch the feed cross-origin, so run this on a
   small server or worker and serve the JSON to the page.
   ============================================================ */
const KINDS = {
...
/* ============================================================
   3. SAMPLE DATA — stands in for the parsed feed
   ============================================================ */
/* Round detail comes from rounds.json, generated by
   scripts/generate-sample-data.py from a simulated Swiss tournament. It is the
   same simulation that produces feed.xml, so the coverage posts and the round
   panel describe the same event.

   Previously every round rendered one hardcoded set of eight pairings, so R1
   and R12 were identical and the round track was decorative. */
let ROUNDS = [];
let eventInfo = null;
...
/* Coverage now comes from this site's own feed rather than a second copy of
   the same posts living here. feed.xml is the single source; the round data
   below it (pairings, standings, feature) is still sample data -- the feed
   carries no round detail. */
...
/* ============================================================
   9. THE HERO STAMP
   ------------------------------------------------------------
   Real now: the time comes from the feed's own <lastBuildDate>
   rather than a hardcoded "2 minutes ago". The label re-renders
   on a slow tick so it ages correctly while the page sits open.
   That ticks the *label* only -- nothing refetches yet, and the
